...
Ext.app.EditFiltersWindow = Ext.extend(Ext.Window, {
	...
	initComponent: function() {
		...
		var config = {
			...
			buttons: [{
				text: "应用",//<SIAT_zh_CN original="Apply">应用</SIAT_zh_CN>
				handler: function() {
					var win = Ext.getCmp(this.windowId);
			...
			layout: "absolute",
			title: "编辑过滤器",//<SIAT_zh_CN original="Edit Filters">编辑过滤器</SIAT_zh_CN>
			width: 510,
	...
	splitFilter: function(record) {
		...
		if (index >= 0) {
			this.filters.splice(index, 1);
			this.summaryPanel.body.dom.innerHTML = '<div class="loading-indicator">正在加载...</div>';//<SIAT_zh_CN original="Loading">正在加载</SIAT_zh_CN>
		}
