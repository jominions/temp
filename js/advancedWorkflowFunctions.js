...
function validateHeatmap()
{
	...
	{
		alert('发现空子集, 需要至少一个子集生成Heatmap');//<SIAT_zh_CN original="Empty subset found, need at least one subset to plot a heatmap">发现空子集, 需要至少一个子集生成Heatmap</SIAT_zh_CN>
		return;
	}
	...
	{
		alert('Comparative marker需要两个非空子集');//<SIAT_zh_CN original="Comparative marker selection requires two non-empty subsets">Comparative marker需要两个非空子集</SIAT_zh_CN>
		return;
	}
...
function validateHeatMapsSample(completedFunction)
{
	...
	if(!jsonDataToPass)
	{
		Ext.Msg.alert("没有子集被载入","请添加记录至子集中以执行工作流!");//<SIAT_zh_CN original="No subsets loaded">没有子集被载入</SIAT_zh_CN><SIAT_zh_CN original="Please add records to a subset to run a workflow">请添加记录至子集中以执行工作流</SIAT_zh_CN>
		return false;
	}
	...
	{
		Ext.Msg.alert("不允许不同案例(Study)之间的比较","您选择来自不同案例的样品, 请清除条目并重新选择来自同一案例的样品.");//<SIAT_zh_CN original="Different study comparisons not allowed">不允许不同案例(Study)之间的比较</SIAT_zh_CN><SIAT_zh_CN original="You have selected samples from different studies. Please clear your entries and select samples from only one study.">您选择来自不同案例的样品, 请清除条目并重新选择来自同一案例的样品.</SIAT_zh_CN>
		return false;	
	}

	if(DataTypeList.size() > 1)
	{
		Ext.Msg.alert("不允许不同案例(Study)之间的比较","您选择来自不同数据类型的样品, 请清除条目并重新选择来自同一数据类型的样品.");//<SIAT_zh_CN original="Different data type comparisons not allowed"></SIAT_zh_CN><SIAT_zh_CN original="You have selected samples from different data types. Please clear your entries and select samples from only one data type.">您选择来自不同数据类型的样品, 请清除条目并重新选择来自同一数据类型的样品.</SIAT_zh_CN>
		return false;	
	}
...
function validateheatmapComplete(result,completedFunction)
{
	...
	{
		Ext.Msg.alert("无数据!","您选择的样品无任何数据!");//<SIAT_zh_CN original="No Data!">无数据!</SIAT_zh_CN><SIAT_zh_CN original="No data found for the samples you selected!">您选择的样品无任何数据!</SIAT_zh_CN>
		return;
	}
...
function finalAdvancedMenuValidation()
{
	...
	if (platformsMatch && !pathwayEmpty && numClustersValid)
	{
		...
		if(!platformsMatch){
			alert('无符合平台');//<SIAT_zh_CN original="Platforms do not match">无符合平台</SIAT_zh_CN>
		}else if(pathwayEmpty){
			alert('请指定路径(pathway)');//<SIAT_zh_CN original="Please specify a pathway before continuing">请指定路径(pathway)</SIAT_zh_CN>
		}else if(!numClustersValid){
			alert('群集数目无效!')//<SIAT_zh_CN original="Number of clusters is invalid!">群集数目无效!</SIAT_zh_CN>
		}
	}	
...
function runVisualizerFromSpan(viewerURL, altviewerURL) {
	...
	Ext.Ajax.request(
	{
		...
		failure: function(result, request){
			//Ext.MessageBox.hide();
			alert('无法取得内容 ' + viewerURL);//<SIAT_zh_CN original="Failed in getting the content of">无法取得内容</SIAT_zh_CN>
		},
		timeout: '1800000'
	});
...
function showGeneSelection()
{
	if(!this.compareGeneSelection)
	{
		...
		title: '比较基因选择',//<SIAT_zh_CN original="Compare Gene Selection">比较基因选择</SIAT_zh_CN>
		layout:'fit',
		width:450,
		autoHeight: true,
		closable: false,
		...
		buttons: [
		          {
		            ...
		            text: '执行工作流',//<SIAT_zh_CN original="Run Workflow">执行工作流</SIAT_zh_CN>
		            handler: function()
		            	{       
		...
		tools:[{
			id:'help',
		qtip:'点击查阅与上下文有关的帮助',//<SIAT_zh_CN original="Click for context sensitive help">点击查阅与上下文有关的帮助</SIAT_zh_CN>
