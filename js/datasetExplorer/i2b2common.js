...
if(el.dom.className=="queryGroupInclude")
	...
	button.firstChild.nodeValue="包含";//<SIAT_zh_CN original="Include">包含</SIAT_zh_CN>
	}
	else
	{
	el.dom.className="queryGroupInclude";
	button.firstChild.nodeValue="排除";//<SIAT_zh_CN original="Exclude">排除</SIAT_zh_CN>
	}
	invalidateSubset(subset);
}
...
function conceptRightClick(event)
{
	...
	},{id: 'setvaluemenu', text: '设置值', handler:function(){showSetValueDialog();}},//<SIAT_zh_CN original="Set Value">设置值</SIAT_zh_CN>
	{
	text: '显示定义', handler:function(){ showConceptInfoDialog(conceptid, conceptid, comment);}
	}//<SIAT_zh_CN original="Show Definition">显示定义</SIAT_zh_CN>
	]
	}); 
	}
	var xy = event.getXY();
...
{
var cleartxt="<div style='text-align:center;font:12pt arial;width:100%;height:100%;'><table style='width:100%;height:100%;'><tr><td align='center' valign='center'>拖拽概念至此面板以查看由该概念分解的子集</td></tr></table></div>";//<SIAT_zh_CN original="Drag concepts to this panel to view a breakdown of the subset by that concept">拖拽概念至此面板以查看由该概念分解的子集</SIAT_zh_CN>
var cleartxt2="<div style='text-align:center;font:12pt arial;width:100%;height:100%;'><table style='width:100%;height:100%;'><tr><td align='center' valign='center'>在菜单中选择 高级->Haploview</td></tr></table></div>";//<SIAT_zh_CN original="Select Advanced->Haploview from the menu">在菜单中选择 高级->Haploview </SIAT_zh_CN>
...
function createNClustersBox(){
	...
	var nclusters = new Ext.form.NumberField({
		...
		validator: function(value)
		{
	        if(value < 1) 
	        {
	            return '您必须选择至少1个聚类';//<SIAT_zh_CN original="You must choose at least 1 cluster.">您必须选择至少1个聚类</SIAT_zh_CN>
	        } 
	        else if(value > 100)
	        {
	        	return '聚类数目须小于100个';//<SIAT_zh_CN original="You must choose less than 100 clusters.">聚类数目须小于100个.</SIAT_zh_CN>
	        }
	        else 
	        {
	            return true;
...
function createPathwaySearchBox(searchInputEltName, divName){
    ...
    var search = new Ext.form.ComboBox({
        ...
        displayField:'title',
        typeAhead: false,
        loadingText: '正在搜索...',//<SIAT_zh_CN original="Searching">正在搜索</SIAT_zh_CN>
        width: 412,
        //listWidth: 430,
        listHeight:500,
...
function createPlatformSearchBox(subsetId, applyToDivIdx){
	...
	var onSelectFn=function(record)
	{
		...
		if (!((!GLOBAL.CurrentPlatforms[0]) || (!GLOBAL.CurrentPlatforms[1]) || (GLOBAL.CurrentPlatforms[0]==GLOBAL.CurrentPlatforms[1]))){
			alert('平台不匹配');//<SIAT_zh_CN original="Platforms do not match">平台不匹配</SIAT_zh_CN>
		}
	}
...
function showCompareStepPathwaySelection()
{
	if(!this.compareStepPathwaySelection)
	{
		compareStepPathwaySelection = new Ext.Window({
                ...
                title: '比较 子集-途径 选择',//<SIAT_zh_CN original="Compare Subsets-Pathway Selection">比较 子集-途径 选择</SIAT_zh_CN>
            	layout:'fit',
                width:450,
                // height:250,
                autoHeight: true,
                ...
                buttons: [
                		{
                            ...
                            text: '运行工作流',//<SIAT_zh_CN original="Run Workflow">运行工作流</SIAT_zh_CN>
                            handler: function(){       
                        ...
                        },
                		{
                            ...
                            text: '应用选择',//<SIAT_zh_CN original="Apply Selections">应用选择</SIAT_zh_CN>
                            handler: function(){       
			   					applyToForm();
			   					return;
			   				}
		        ...
		        tools:[{
					id:'help',
					qtip:'点击以获取上下文敏感的相关帮助',//<SIAT_zh_CN original="Click for context sensitive help">点击以获取上下文敏感的相关帮助</SIAT_zh_CN>
...
if(!this.compareStepPathwaySelectionRBM)
{
	compareStepPathwaySelectionRBM = new Ext.Window({
                ...
                title: '基于RBM平台的比较 子集-途径 选择',//<SIAT_zh_CN original="Compare Subsets-Pathway Selection for RBM Platform">基于RBM平台的比较 子集-途径 选择</SIAT_zh_CN>
            	layout:'fit',
                width:450,
                // height:250,
                autoHeight: true,
...
for(var i=1;i<=GLOBAL.NumOfQueryCriteriaGroups;i++)
	{
		...
		if(qcd.dom.childNodes.length>0)
		{
			...
			{
				query=query+"<br>且<br>";
			}//<SIAT_zh_CN original="AND">且</SIAT_zh_CN>
			query=query+panel;
		}
	}
return query;
...
if(invert==1)
	{
	panel=panel+"<b>非 (</b>";//<SIAT_zh_CN original="NOT">非</SIAT_zh_CN>
	}
else
	{
	panel=panel+"<b>(</b>";
...
for(var i=0;i<qd.childNodes.length;i++)
{
	if(i>0)
		{
		panel=panel+'<br><b>或</b><br>';//<SIAT_zh_CN original="OR">或</SIAT_zh_CN>
		}
...
if(access!=undefined)
  			...
  			newnode.disable();
  			newnode.on('beforeload', function(node){alert("该节点限制访问。请联系您的管理员以获得访问权限。"); return false});		
  			}//<SIAT_zh_CN original="Access to this node has been restricted. Please contact your administrator for access.">该节点限制访问。请联系您的管理员以获得访问权限。</SIAT_zh_CN>
  		   }
}
