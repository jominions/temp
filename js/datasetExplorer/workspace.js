...
function showSaveSubsetsDialog(){
    ...
    {
        alert('检测到空子集，需要至少一个有效子集以保存一个比较');//<SIAT_zh_CN original="Empty subsets found, need at least 1 valid subset to save a comparsion">检测到空子集，需要至少一个有效子集以保存一个比较</SIAT_zh_CN>
        return;
    }
	...
	jQuery( "#saveSubsetsDialog" ).dialog({title: '保存子集', modal:true});//<SIAT_zh_CN original="Save Subsets">保存子集</SIAT_zh_CN>
...
function saveSubsets(subsetDescription, isSubsetPublic){
	...
	jQuery.ajax({
		  ...
		  failure:function(data){alert("子集保存失败");},//<SIAT_zh_CN original="Subsets failed to save.">子集保存失败</SIAT_zh_CN>
...
function initWorkspaceDataTables(){
	...
	*/
    ...
    jQuery( "#subsets_length" ).before( "<div id=\"subset_manager_name\"><label>子集管理器</label></div>" );//<SIAT_zh_CN original="Subset Manager">子集管理器</SIAT_zh_CN>
    jQuery( "#reports_length" ).before( "<div id=\"report_manager_name\"><label>报告管理器</label></div>" );//<SIAT_zh_CN original="Report Manager">报告管理器</SIAT_zh_CN>
}
...
function applySubsets(subsetId, study){
	...
	if(!isSubsetEmpty(1) || !isSubsetEmpty(2)){
		overrideCurrentSubsets = confirm("这将覆盖您刚才在比较栏位中选择的条件。")//<SIAT_zh_CN original="This will override the criteria you have selected on the comparison tab.">这将覆盖您刚才在比较栏位中选择的条件。</SIAT_zh_CN>
	}
...
function deleteSubset(subsetId){
	if(confirm("子集将被删除。继续吗？")){//<SIAT_zh_CN original="Subset will be deleted. Proceed?">子集将被删除。继续吗？</SIAT_zh_CN>
		...
		.fail(function(){alert("删除子集时服务器出现错误");});//<SIAT_zh_CN original="Server Error while deleting subset">删除子集时服务器出现错误</SIAT_zh_CN>
	}
}
...
function togglePublicFlag(subsetId){
	...
	}).fail(function() { alert("切换公有标志时服务器出现错误"); })//<SIAT_zh_CN original="Server Error in toggling public flag">切换公有标志时服务器出现错误</SIAT_zh_CN>
}
