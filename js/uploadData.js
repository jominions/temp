...
function showDataUploadForm() {
	...
	if ($j('#study').val() == null || $j('#study').val() == '') {
		$j('#studyErrors').html('<div class="fieldError">请选择一个案例(Study)</div>');//<SIAT_zh_CN original="Please select a study">请选择一个案例(Study)</SIAT_zh_CN>
		errors = true;
	}
	if ($j('#dataType').val() == null || $j('#dataType').val() == "null" || $j('#dataType').val() == '') {
		$j('#dataTypeErrors').html('<div class="fieldError">请选择一种分析类型</div>');//<SIAT_zh_CN original="Please select an analysis type">请选择一种分析类型</SIAT_zh_CN>
		errors = true;
	}
	if ($j('#analysisName').val() == null || $j('#analysisName').val() == '') {
		$j('#analysisNameErrors').html('<div class="fieldError">请输入分析名称</div>');//<SIAT_zh_CN original="Please enter an analysis name">请输入分析名称</SIAT_zh_CN>
		errors = true;
	}
	...
	$j('#dataFormTitle2').html('上传 ' + title + ' 数据')//<SIAT_zh_CN original="Upload">上传</SIAT_zh_CN><SIAT_zh_CN original="Data">数据</SIAT_zh_CN>
...
function validateFile(event) {
    ...
    if ($j('#uploadFile').val() == null || $j('#uploadFile').val() == '') {
        $j('#uploadFileErrors').html('<div class="fieldError">请选择一个档案上传</div>');//<SIAT_zh_CN original="Please select a file to upload">请选择一个档案上传</SIAT_zh_CN>
        errors = true;
    }
    ...
    $j.ajax({
        ...
        success: function(response) {
            if (!response.found) {
                ...
                $j('#studyNoFolderMessage').html("取得此案例的文件夹发生错误: " + response.message + ". 请点击右上方按钮联系管理员.");//<SIAT_zh_CN original="Error retrieving folder for this study">取得此案例的文件夹发生错误</SIAT_zh_CN><SIAT_zh_CN original="Please contact the administrator using the button on the top right">请点击右上方按钮联系管理员</SIAT_zh_CN>
            }
        },
        failure: function(xhr) {
            if (xhr.responseText != null && xhr.responseText != "") {
                $j('#studyNoFolderMessage').html("取得此案例的文件夹发生错误: " + xhr.responseText + ". 请点击右上方按钮联系管理员.");//<SIAT_zh_CN original="Error retrieving folder for this study">取得此案例的文件夹发生错误</SIAT_zh_CN><SIAT_zh_CN original="Please contact the administrator using the button on the top right">请点击右上方按钮联系管理员</SIAT_zh_CN>
            }
        }
    });
      ...
      jQuery('#divBrowseStudies').dialog(
                  {
                        ...
                        height: 300,
                        width: 500,
                        title: "案例(Study)在 " + (UPLOAD_STUDY_TYPE == 'i2b2'?"数据集管理器":"分面搜索"),//<SIAT_zh_CN original="Studies in">案例(Study)在</SIAT_zh_CN><SIAT_zh_CN original="Dataset Explorer">数据集管理器</SIAT_zh_CN><SIAT_zh_CN original="faceted search">分面搜索</SIAT_zh_CN>
                        show: 'fade',
                        hide: 'fade',
...
function downloadTemplate() {
	...
	if ($j('#dataType').val() == null || $j('#dataType').val() == "null" || $j('#dataType').val() == '') {
		$j('#dataTypeErrors').html('<div class="fieldError">请选择一个分析类型</div>');//<SIAT_zh_CN original="Please select an analysis type">请选择一个分析类型</SIAT_zh_CN>
...
jQuery(document).ready(function() {
    ...
    jQuery('body').on('click', '.sidebarRadio', function() {
        ...
        if (jQuery(this).attr('id') == 'uploadAnalysisRadio') {
            ...
            jQuery('.dataFormTitle').text("上传分析数据");//<SIAT_zh_CN original="Upload Analysis Data">上传分析数据</SIAT_zh_CN>
        }
        else {
            if (jQuery(this).attr('id') == 'uploadFileDatasetExplorerRadio') {
                UPLOAD_STUDY_TYPE = 'i2b2';
                jQuery('.dataFormTitle').text("上传档案到数据集管理器");//<SIAT_zh_CN original="Upload File to Dataset Explorer">上传档案到数据集管理器</SIAT_zh_CN>
            }
            else if (jQuery(this).attr('id') == 'uploadFileRadio') {
                UPLOAD_STUDY_TYPE = 'Experiment';
                jQuery('.dataFormTitle').text("上传档案到分面搜索");//<SIAT_zh_CN original="Upload File to Faceted Search">上传档案到分面搜索</SIAT_zh_CN>
            }
