...
function updateForNewFolder(folderId)	{
	jQuery('#metadata-viewer').load(folderDetailsURL + '?id=' + folderId, {}, function() {
		...
		jQuery.ajax({
			...
			error: function(xhr) {
				console.log('错误! 状态 = ' + xhr.status + xhr.statusText);//<SIAT_zh_CN original="Error!  Status">错误! 状态</SIAT_zh_CN>
			}
		});
...
function showIEWarningMsg(){
	...
	if (jQuery.browser.msie && jQuery.browser.version.substr(0,1)<9) {
		...
		var msg = "<div id='IEwarningBox'>您使用的浏览器未被支持, 请使用最新版本的Chrome浏览器. <br /><br />";//<SIAT_zh_CN original="Your browser is not supported. Please use the latest version of Chrome">您使用的浏览器未被支持, 请使用最新版本的Chrome浏览器</SIAT_zh_CN>
			msg = msg + "<a href='#' id='IEwarningOverlayLink'>More info</a> | <a href='#' onclick=\"javascript:jQuery('#IEwarningBox').slideUp('fast');\">Close</a> </div>";//<SIAT_zh_CN original="More info">更多信息</SIAT_zh_CN><SIAT_zh_CN original="Close">关闭</SIAT_zh_CN>
	
		var overlayMsg = "<div style='padding:20px'><p>TranSMART 分面搜索使用特定网站技术, 部份旧版本浏览器并未支持. ";//<SIAT_zh_CN original="TranSMART Faceted Search uses certain web technologies that are not fully supported by older browsers">TranSMART 分面搜索使用特定网站技术, 部份旧版本浏览器并未支持</SIAT_zh_CN>
			overlayMsg = overlayMsg + "建议使用Google Chrome浏览器 </p><br /><p>To request Chrome, send an SRM request " ;//<SIAT_zh_CN original="Google Chrome is the preferred browser within J&J to use with tranSMART">建议使用Google Chrome浏览器</SIAT_zh_CN>
...
function exportLinePlotData(analysisId, exportType)
{
	...
	case 'image':
		...
		break;
	default:
		console.log('错误 - 无效的汇出选项');//<SIAT_zh_CN original="Error - invalid Export option">错误 - 无效的汇出选项</SIAT_zh_CN>
	}
...
function analysisMenuEvent(id){
	...
	default:
		...
		console.log("无效选项: " +id);//<SIAT_zh_CN original="Invalid option">无效选项</SIAT_zh_CN>	
	}
...
function showBoxOrLinePlotVisualization(panel, analysisID, isBoxplot)	{
	...
	if(redraw) { 
		jQuery("#analysis_holder_" + analysisID).mask("载入中...");//<SIAT_zh_CN original="Loading">载入中</SIAT_zh_CN>
...
function updateLineplot(analysisID){
	...
	if(data == ''){
		console.log("错误: 找不到数据");//<SIAT_zh_CN original="Error: Could not find data">错误: 找不到数据</SIAT_zh_CN>
	}else
		{
...
//	cohortDisplayStyles = [''].concat(cohortDisplayStyles);
	...
	if(hasFoldChange){
	    ...
	    vis.add(pv.Label)
	    	...
	    	.text("Fold change");//<SIAT_zh_CN original="Fold change">倍数变化(Fold change)</SIAT_zh_CN>
	...
	if(hasTPvalue)
	{
	    ...
	    vis.add(pv.Label)
			...
			.textAlign("left")
		    .font("bold 11px sans-serif")
			.text("TEA p值");//<SIAT_zh_CN original="TEA p-value">TEA p值</SIAT_zh_CN> 
	...
	if(hasPvalue){
	    ...
	    vis.add(pv.Label)
			...
			.textAlign("left")
		    .font("bold 11px sans-serif")
			.text("p值");//<SIAT_zh_CN original="p-value">p值</SIAT_zh_CN>
    ...
    vis.add(pv.Label)
    	...
    	.textBaseline("middle")
    	.title("View in boxplot")//<SIAT_zh_CN original="View in boxplot">View in boxplot</SIAT_zh_CN>
    	.text(function(d)	{
    		return d.PROBE;
    	});
	...
	vis.add(pv.Bar)
		...
		.add(pv.Label)
		.textStyle("white")
		.text("最小值: "+Math.round((rangeMin)*100)/100);//Math.round get nearest integer//<SIAT_zh_CN original="min">最小值</SIAT_zh_CN>	
	...
	vis.add(pv.Bar)
		...
		.add(pv.Label)
		.textStyle("white")
		.text("最大值: " + Math.round((rangeMax)*100)/100);//Math.round get nearest integer//<SIAT_zh_CN original="max">最大值</SIAT_zh_CN>
...
function openSaveSearchDialog()  {
	...
	if (keywords.length>0)  {
		...
		alert("没有搜索条件可储存!")//<SIAT_zh_CN original="No search criteria to save">没有搜索条件可储存</SIAT_zh_CN>
	}
	...
	rwgAJAXManager.add({
		...
		success: function(response) {
			...
			if (response['success'])  {
				...
				alert('失败');// show message from server//<SIAT_zh_CN original="failed">失败</SIAT_zh_CN>
			}
 		},
		error: function(xhr) {
			console.log('错误! 状态 = ' + xhr.status + xhr.statusText);//<SIAT_zh_CN original="Error!  Status">错误! 状态</SIAT_zh_CN>
	...
	jQuery("#pagination_" + analysisId).paging(numberOfPages, { 
        ...
        onFormat: function(type) {      
                ...
                case 'next':      
                        ...
                        if (this.active) {      
                                return '<a href="#' + this.value + '" class="next">下一个 &raquo;</a>'; //<SIAT_zh_CN original="Next">下一个</SIAT_zh_CN>      
                        }      
                        return '<span class="disabled">下一个 &raquo;</span>';//<SIAT_zh_CN original="Next">下一个</SIAT_zh_CN>    
      
                case 'prev':      
      
                        if (this.active) {      
                                return '<a href="#' + this.value + '" class="prev">&laquo; 前一个</a>';      
                        }      
                        return '<span class="disabled">&laquo; 前一个</span>';//<SIAT_zh_CN original="Previous">前一个</SIAT_zh_CN>     
...
function updateSelectedAnalyses() {
	...
	if (selectedboxes.length > 0) {
		jQuery('#selectedAnalyses').html("<b>(" + selectedboxes.length + "</b> 分析已选)");//<SIAT_zh_CN original="analyses selected">分析已选</SIAT_zh_CN>
	}
	else {
...
function startPlotter() {
	var selectedboxes = jQuery(".analysischeckbox:checked");
	if (selectedboxes.length == 0) {
		alert("未选择任何分析, 请选择分析继续进行绘制.");//<SIAT_zh_CN original="No analyses are selected! Please select analyses to plot">未选择任何分析, 请选择分析继续进行绘制</SIAT_zh_CN>
	}
	else {
		var analysisIds = "";
