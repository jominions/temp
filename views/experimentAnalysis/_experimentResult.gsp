...
<g:else>
    ...
    <p style="font-weight:bold; font-size:11px;padding-left:5px;padding-bottom:5px; padding-top:5px;">
        <g:if test="${searchresult?.result?.groupByExp}">案例结果:<!--<SIAT_zh_CN original="Study result">案例结果</SIAT_zh_CN>-->&nbsp;&nbsp;${searchresult?.result?.analysisCount}
            <g:if test="${searchresult?.result?.analysisCount > 1}">分析<!--<SIAT_zh_CN original="analyses">分析</SIAT_zh_CN>--></g:if>
            <g:else>分析<!--<SIAT_zh_CN original="analysis">分析</SIAT_zh_CN>--></g:else>
            &nbsp;从<!--<SIAT_zh_CN original="from">从</SIAT_zh_CN>--> ${searchresult?.result?.expCount} 实验<!--<SIAT_zh_CN original="experiment(s)">实验</SIAT_zh_CN>-->
        </g:if>
        <g:else>
            结果分析<!--<SIAT_zh_CN original="Analysis result">结果分析</SIAT_zh_CN>-->:&nbsp;&nbsp;${searchresult?.result?.analysisCount}
            <g:if test="${searchresult?.result?.analysisCount > 1}">分析<!--<SIAT_zh_CN original="analyses">分析</SIAT_zh_CN>--></g:if>
            <g:else>分析<!--<SIAT_zh_CN original="analyses">分析</SIAT_zh_CN>--></g:else>
            &nbsp;从<!--<SIAT_zh_CN original="from">从</SIAT_zh_CN>--> ${searchresult?.result?.expCount} 实验<!--<SIAT_zh_CN original="experiment">实验</SIAT_zh_CN>-->
            <g:if test="${teaDisplay}">&nbsp;[${ear.analysisCount - ear.inSignificantAnalCount} Significant TEA / ${ear.inSignificantAnalCount} Insignificant TEA]
                <br><span
                    style="color:red;">注意，此处仅显示重要的TEA分析!<!--<SIAT_zh_CN original="Note, only significant TEA Analyses are displayed">注意，此处仅显示重要的TEA分析</SIAT_zh_CN>--></span>
            </g:if>
        </g:else>
    </p>
                            ...
                            <table width="100%">
                                <tr style="padding-bottom: 5px">
                                    <td style="padding: 5px 0px 10px 5px; margin-top: 5px;"><div
                                        ...
                                        <g:if test="${expAnalysisResult.analysisCount > 1}">分析发现<!--<SIAT_zh_CN original="analyses found">分析发现</SIAT_zh_CN>--></g:if>
                                        <g:else>分析发现<!--<SIAT_zh_CN original="analysis found">分析发现</SIAT_zh_CN>--></g:else>
                                        &nbsp;&nbsp;&nbsp;
